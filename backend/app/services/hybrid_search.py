...
def _bm25_search(question: str, top_n: int) -> List[str]:
...
def _vector_search(question_embedding: List[float], top_n: int) -> List[str]:
    
    results = query_similar_chunks(question_embedding, top_n)
...
def hybrid_retrieve(question: str, question_embedding: List[float],
                     candidate_pool_size: int = 20) -> dict:
    
    vector_ranked_ids = _vector_search(question_embedding, candidate_pool_size)
    bm25_ranked_ids = _bm25_search(question, candidate_pool_size)
